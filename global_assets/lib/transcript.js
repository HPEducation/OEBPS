if (typeof UIWebView === "undefined") {
	if($("#legacyTranscript").length != 0) {
		...
		$("#legacyTranscript").hide();
		transcriptTxt = $("#legacyTranscript").html();

		if (transcriptTxt.trim()) {
			// is not empty nor whitespace
			console.log("Transcript not empty");

			if (typeof window.transcriptAdded === "undefined") {
				...
				function enableTranscript (b) {
					if (b != false) {
						console.log("transcript panel enabled.")
						$(".transcriptbtntoggle").css("visibility", "visible");
					} else {
						console.log("transcript panel disabled.")
